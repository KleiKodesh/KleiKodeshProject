...
**Frequently visited folders.** `stores/frequentFoldersStore.ts` counts which folders files are opened from and ranks them by *time-decayed frequency*, not recency: each visit adds a point and accumulated points halve every 14 days, so a folder used daily outranks one hammered once last month. Scores are decayed lazily (each entry records when its score was last computed) — there is no sweep, and a long-closed app still comes back correctly aged. The top four appear as tiles between the static tiles and the recents, carrying the same pin and trash actions; clicking one opens the file dialog already pointed at that folder. Visits are recorded in `tabStore.trackTabNavigation`, which deliberately skips HebrewBooks downloads — those land in our own cache directory, which the user never chose.
