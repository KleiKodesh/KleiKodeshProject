...
**Page & UI:**
- `PdfViewPage.vue` — main PDF viewer page; manages iframe lifecycle, toolbar visibility, and OCR mode toggle; propagates the app-wide scrollbars mode (static / auto-hide — Ctrl+Shift+H, reading mode, or settings) into the viewer iframe via `useIframeScrollbarsAutoHide` (same-origin style injection + scroll-activity listener)
