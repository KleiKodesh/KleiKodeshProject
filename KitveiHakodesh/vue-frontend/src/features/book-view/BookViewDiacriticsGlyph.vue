...
/**
 * The diacritics-cycle glyph, one per stage of the cycle. Its own component only so the
 * toolbar button and the row it collapses into (BookViewToolbarOverflowMenu) draw the same
 * glyph from one source rather than two copies of three inline SVGs.
 *
 * Colour is inherited, not set: theme.css pins `svg { color }`, which otherwise cuts the
 * glyph off from the state colour its button carries.
 */
