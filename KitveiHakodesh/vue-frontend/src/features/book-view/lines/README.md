...
**useBookViewLineLink.ts** - the "העתק קישור לקטע זה" context menu action. Builds a `seforimapp://book/<bookId>?index=<lineIndex>` deep link — deliberately shaped like the `otzaria://open/book/<bookId>?index=<lineIndex>` links `HostLink.cs` already parses, with `index` meaning a 0-based positional line index in both — for the line the user pressed to open the menu (recorded on pointerdown, since both right-click and long-press begin with one) and copies it to the clipboard with a toast. The app does not register a handler for the `seforimapp://` scheme — the format is future-proofing only; `buildLineLink` is the single place it is defined.
