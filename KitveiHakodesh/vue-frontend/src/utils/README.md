...
**censorDivineNames.ts** — censors divine names according to the `divineNameMode` setting. The tetragrammaton rendering is mode-selected (`yudDaled` ידוד, `yudKuf` יקוק, `doubleYud` יי, `heApostrophe` ה' — nikkud dropped, cantillation kept — or `none`); the other names (אדנ-י, א-להים, א-ל, ש-די, י-ה) get the dash treatment in every mode except `none`. Exports `DIVINE_NAME_MODE_OPTIONS` for the settings UI and `normalizeDivineNameMode` for migrating the legacy boolean setting.
