...
**fontsApi.ts** — `detectAvailableFonts()`: the machine's real Hebrew-capable font families, for the settings font picker. Returns TWO lists: `fonts` (every Hebrew family) and `teamimFonts` (the subset whose cmap also covers the cantillation marks, U+0591-05AF). The te'amim picker passes `teamim-only` to `FontSelector` and gets the narrow list — a family that cannot draw the marks would render a hasTeamim book with them dropped. One enumeration answers both; the backend walks each face's cmap once and probes two codepoints. The app's own bundled fonts (`BUNDLED_FONTS`, shipped in `public/fonts/` and declared `@font-face` in `main.css`) are prepended to every path and de-duplicated against it — the OS enumerators cannot see a web font, so this is the only way they reach the picker. The "ברירת מחדל" badge is NOT decided here — each picker badges its own default family (`DEFAULT_TEXT_FONT_FAMILY` / `DEFAULT_TEAMIM_FONT_FAMILY` from settingsStore), since one dropdown's default is another's ordinary choice. Asks the C# host (`getFonts` → DirectWrite, Helpers/FontsProvider) when hosted, the service (`getFonts` → its TWIN HebrewFontsProvider) in dev, and falls back to `fontsCanvasProbe.ts` only when neither answers — that probe measures text width and cannot read a cmap, so on that path `teamimFonts` is the bundled faces alone rather than a guess about the machine's. Deliberately uncached: `FontSelector.vue` calls it live on every dropdown open (showing a loading row meanwhile), so long-running sessions always see the machine's current fonts — installs and removals included. `isHosted` is TRUE in dev and cannot pick the path — branch on `__webviewAction`.
