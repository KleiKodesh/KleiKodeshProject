...
**fontsApi.ts** — `detectAvailableFonts()`: the machine's real Hebrew-capable font families, for the settings font picker. The app's own bundled fonts (`BUNDLED_FONTS`, shipped in `public/fonts/` and declared `@font-face` in `main.css`) are prepended to every path and de-duplicated against it — the OS enumerators cannot see a web font, so this is the only way they reach the picker. The "ברירת מחדל" badge is NOT decided here — each picker badges its own default family (`DEFAULT_TEXT_FONT_FAMILY` / `DEFAULT_TEAMIM_FONT_FAMILY` from settingsStore), since one dropdown's default is another's ordinary choice. Asks the C# host (`getFonts` → DirectWrite, Helpers/FontsProvider) when hosted, the service (`getFonts` → its TWIN HebrewFontsProvider) in dev, and falls back to `fontsCanvasProbe.ts` only when neither answers. Deliberately uncached: `FontSelector.vue` calls it live on every dropdown open (showing a loading row meanwhile), so long-running sessions always see the machine's current fonts — installs and removals included. `isHosted` is TRUE in dev and cannot pick the path — branch on `__webviewAction`.
