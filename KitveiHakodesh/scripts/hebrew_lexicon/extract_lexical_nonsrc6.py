...
Outputs (scratchpad): lexN_review_<start>.txt , lex_cand_<start>.json
"""
...
def main():
    ...
    src6=sorted({h for (h,) in d.execute("""SELECT DISTINCT w.headword FROM sense s JOIN word w ON w.id=s.word_id
       WHERE w.headword NOT IN (SELECT DISTINCT w2.headword FROM sense s2 JOIN word w2 ON w2.id=s2.word_id WHERE s2.source_id=6)""")})
    ...
    c.close()
    json.dump(cand, open(os.path.join(SP,f"lexN_cand_{start}.json"),"w",encoding="utf-8"), ensure_ascii=False, indent=1)
    open(os.path.join(SP,f"lexN_review_{start}.txt"),"w",encoding="utf-8").write(
        f"NON-source6 bases {start}..{start+count} with lexical candidates: {len(cand)}  (total forms: {sum(len(v) for v in cand.values())})\n\n"+"\n".join(review))
    print(f"bases with candidates: {len(cand)}  forms: {sum(len(v) for v in cand.values())}  -> lexN_review_{start}.txt / lexN_cand_{start}.json")
